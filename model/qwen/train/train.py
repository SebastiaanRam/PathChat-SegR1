...
import torch
import transformers
from qwen import conversation as conversation_lib
from qwen.constants import (DEFAULT_IM_END_TOKEN, DEFAULT_IM_START_TOKEN,
                             DEFAULT_IMAGE_TOKEN, IGNORE_INDEX,
                             IMAGE_TOKEN_INDEX)
from qwen.mm_utils import tokenizer_image_token
from qwen.model import *
from qwen.model.language_model.llava_llama import *
from qwen.train.llava_trainer import LLaVATrainer
...
class QwenTrainer(LLaVATrainer):  # 继承修改 (#! Qwen Mod)
        def _save(self, output_dir: Optional[str] = None, state_dict=None):
            # 修改保存逻辑以适应 Qwen 结构
            if getattr(self.args, "tune_mm_mlp_adapter", False):
                # 仅保存适配器权重
                keys_to_match = ["gate_proj", "down_proj", "up_proj"]
                weight_to_save = get_mm_adapter_state_maybe_zero_3(
                    self.model.named_parameters(), keys_to_match
                )
                torch.save(weight_to_save, os.path.join(output_dir, "qwen_lora.bin"))
            else:
...
def train():
    ...
    training_args.remove_unused_columns = False  # 必须保留未使用列
    training_args.gradient_checkpointing = True  # 推荐启用梯度检查点
    if model_args.version is None:
        model_args.version = "qwen"  # 设置默认对话模板版本
    ...
    tokenizer = transformers.AutoTokenizer.from_pretrained(
        ...
        padding_side="right",
        use_fast=False,
        trust_remote_code=True  # 必须启用 (#! Qwen Mod)
    )

    qwen_special_tokens = {"additional_special_tokens": ["<image>", "</image>"]}
    num_added = tokenizer.add_special_tokens(qwen_special_tokens)
    if num_added > 0:
        model.resize_token_embeddings(len(tokenizer))
        # 初始化新 token 的 embedding
        with torch.no_grad():
            input_embeds = model.get_input_embeddings().weight.data
            output_embeds = model.get_output_embeddings().weight.data
            avg_embed = input_embeds[:-num_added].mean(dim=0)
            input_embeds[-num_added:] = avg_embed
            output_embeds[-num_added:] = avg_embed
